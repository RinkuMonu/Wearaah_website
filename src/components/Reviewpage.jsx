...
const Toast = ({ toast }) => {
  ...
  return (
    <div className={`fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-5 py-3 rounded-full shadow-xl text-sm font-medium transition-all
...
const ReviewModal = ({ item, existingReview, onClose, onSuccess }) => {
  ...
  const setBreakdown = (key, val) =>
    setForm((f) => ({ ...f, ratingBreakdown: { ...f.ratingBreakdown, [key]: val } }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        ratingBreakdown: form.ratingBreakdown,
      };
      if (isEdit) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10 rounded-t-2xl">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5 line-clamp-1">{item.productName} · {item.size} · {item.color}</p>
          </div>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#927f68]/30 focus:border-[#927f68] transition-all resize-none"
            />
          </div>

          {/* Breakdown */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-3">Detailed Ratings</label>
...
/* ═══════════════════════════════════════════════════════════
   REVIEW CARD (existing review display)
═══════════════════════════════════════════════════════════ */
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-full bg-[#927f68] flex items-center justify-center text-white text-sm font-bold shrink-0">
            {review.title?.[0]?.toUpperCase() || "R"}
          </div>
          <div>
            <p className="text-sm font-semibold text-gray-900 leading-tight">{review.title || "Review"}</p>
            <p className="text-[11px] text-gray-400 mt-0.5">
              {new Date(review.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 bg-emerald-500 text-white text-xs font-bold px-2.5 py-1 rounded-full shrink-0">
      ...
      {(bd.fit || bd.quality || bd.valueForMoney) ? (
        <div className="flex flex-wrap gap-2 mb-3">
          {bd.fit > 0 && <span className="text-[11px] bg-amber-50 border border-amber-100 text-amber-700 px-2.5 py-1 rounded-full font-medium">Fit: {bd.fit}/5</span>}
          {bd.quality > 0 && <span className="text-[11px] bg-amber-50 border border-amber-100 text-amber-700 px-2.5 py-1 rounded-full font-medium">Quality: {bd.quality}/5</span>}
          {bd.valueForMoney > 0 && <span className="text-[11px] bg-amber-50 border border-amber-100 text-amber-700 px-2.5 py-1 rounded-full font-medium">Value: {bd.valueForMoney}/5</span>}
        </div>
      ) : null}
...
/* ═══════════════════════════════════════════════════════════
   ORDER ITEM ROW (pick what to review)
═══════════════════════════════════════════════════════════ */
const OrderItemRow = ({ item, order, existingReview, onWriteReview }) => (
  <div className="flex items-center justify-between gap-4 p-4 bg-gray-50 hover:bg-amber-50/50 rounded-xl border border-gray-100 hover:border-amber-200 transition-all group">
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[10px] text-gray-400 mt-0.5">Order #{order.orderNumber}</p>
      </div>
    </div>

    {existingReview ? (
      <div className="flex items-center gap-2 shrink-0">
        <div className="flex items-center gap-1 bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-semibold px-2.5 py-1 rounded-full">
          <FiCheck size={10} /> Reviewed
        </div>
        <button
          onClick={() => onWriteReview(item, order, existingReview)}
          className="text-xs text-[#927f68] hover:text-[#7a6650] font-medium px-2 py-1 rounded-lg hover:bg-[#927f68]/10 transition-all"
        >
          Edit
        </button>
      </div>
      ...
      <button
        onClick={() => onWriteReview(item, order, null)}
        className="shrink-0 flex items-center gap-1.5 text-xs font-semibold text-[#927f68] border border-[#927f68]/40 hover:border-[#927f68] hover:bg-[#927f68] hover:text-white px-3 py-1.5 rounded-xl transition-all group-hover:shadow-sm"
      >
...
const currentUserEmail = user?.email || user?.user?.email;
  ...
  // Reviews state — keyed by variantId
  ...
  const fetchOrders = useCallback(async (page = 1) => {
    ...
    try {
      ...
      // Fetch reviews for all unique variantIds
      const variantIds = [
        ...new Set(
  ...
  /* ── Fetch reviews for a variant ── */
  const fetchReviewsForVariant = useCallback(async (variantId) => {
    if (!variantId) return;
    setReviewsLoading((prev) => ({ ...prev, [variantId]: true }));
    try {
      ...
      setReviewsLoading((prev) => ({ ...prev, [variantId]: false }));
    }
  }, []);

  useEffect(() => { fetchOrders(1); }, [fetchOrders]);

  /* ── Flat list of all order items with review status ── */
const allItems = orders.flatMap((order) =>
  ...
  const pendingItems   = allItems.filter((x) => !x.review);
  const reviewedItems  = allItems.filter((x) => x.review);

  /* ── Like / Unlike ── */
  const handleLike = async (reviewId, action) => {
    try {
      await api.post(`/review/like/${reviewId}`, { action });
      // Re-fetch reviews for all variants to sync likes
      const variantIds = Object.keys(reviewsByVariant);
      variantIds.forEach((vid) => fetchReviewsForVariant(vid));
    } catch {
  ...
  return (
    <>
      <style>{`
        ...
        .tab-active   { color: #927f68; border-bottom: 2px solid #927f68; }
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Cancel</button>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-white border-b border-gray-100 sticky top-0 z-30">
          <div className="w-full mx-auto px-4 sm:px-6 py-4">
            <div className="flex items-center gap-3">
              <button onClick={() => navigate(-1)} className="w-9 h-9 flex items-center justify-center rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-500 transition-colors">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{totalReviews} review{totalReviews !== 1 ? "s" : ""} written</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Total Orders", value: orders.length + (currentPage > 1 ? "+" : "") },
              { label: "Reviewed",     value: totalReviews,           accent: true },
              { label: "Pending",      value: pendingItems.length },
            ].map(({ label, value, accent }) => (
              <div key={label} className={`rounded-2xl p-4 text-center border ${accent ? "bg-[#927f68] border-[#927f68] text-white" : "bg-white border-gray-100"}`}>
                <p className={`text-2xl font-bold ${accent ? "text-white" : "text-gray-900"}`}>{value}</p>
                <p className={`text-xs mt-0.5 ${accent ? "text-white/80" : "text-gray-500"}`}>{label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="flex border-b border-gray-100">
              {[
                ...
                { id: "reviewed", label: "My Reviews",       count: totalReviews },
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex-1 py-3.5 text-sm font-semibold transition-all flex items-center justify-center gap-2
                    ${activeTab === tab.id ? "tab-active bg-amber-50/50" : "tab-inactive hover:text-gray-600"}`}
                >
                  {tab.label}
                ...
                activeTab === "pending" ? (
                    ...
                    <div className="space-y-3">
                      {pendingItems.map(({ item, order }) => (
                        <OrderItemRow
                          ...
                          order={order}
                          existingReview={null}
                          onWriteReview={(i, o, er) => setModal({ item: i, order: o, existingReview: er })}
                        />
                      ))}
                    </div>
                  )
                  ...
                  reviewedItems.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {reviewedItems.map(({ item, order, review }) => (
